...
const stripCheckoutSession = catchAsyncErrors(async (req, res) => {
    ...
    // Get room details
    const vehicle = await Vehicle.findById(req.query.roomId);

    const { startDate, endDate, daysOfRent } = req.query;
    ...
    const session = await stripe.checkout.sessions.create({
        ...
        cancel_url: `${origin}/vehicle/${vehicle._id}`,
        customer_email: req.user.email,
        client_reference_id: req.query.roomId,
        metadata: { startDate, endDate, daysOfRent },
        line_items: [
            {
                name: vehicle.name,
